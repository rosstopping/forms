    ...
    <form method="POST" action="{{ route('admin.website-builder.store') }}" class="space-y-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div class="grid gap-5 md:grid-cols-2">
            ...
            <div>
                ...
                <select id="github_installation_id" name="github_installation_id" required class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"><option value="">Select an account</option>@foreach ($installations as $installation)<option value="{{ $installation->id }}" @selected((string) old('github_installation_id') === (string) $installation->id) @disabled(($installation->permissions['administration'] ?? null) !== 'write')>{{ $installation->account_login }}{{ ($installation->permissions['administration'] ?? null) !== 'write' ? ' — needs Administration: write' : '' }}</option>@endforeach</select>
                ...
                @if ($installations->contains(fn ($installation) => ($installation->permissions['administration'] ?? null) !== 'write'))<p class="mt-1 text-xs text-amber-700">In the GitHub App settings, set Repository permissions → Administration to Read and write, then approve the updated permission on the installation.</p>@endif
            </div>
        </div>
