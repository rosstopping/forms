...
<div class="space-y-6">
    <header class="space-y-3">
        ...
        <p class="text-slate-600 text-base sm:text-sm">Compared with {{ $audit->domain }} · {{ strtoupper($audit->language_code) }} · Location {{ $audit->location_code }} · {{ ucfirst(str_replace('_', ' ', $audit->status)) }}</p>
        <p class="text-slate-500 text-base sm:text-sm">Third-party estimates, not Search Console measurements. Collected {{ $audit->started_at?->format('j M Y, H:i') ?? 'Pending' }}.</p>
        <p class="text-slate-500 text-base sm:text-sm">Sample limits: {{ $audit->limits['ranked_keywords'] }} ranked, {{ $audit->limits['shared_keywords'] }} shared and {{ $audit->limits['missing_keywords'] }} missing keywords; {{ $audit->limits['leading_pages'] }} leading pages and {{ $audit->limits['analyse_pages'] }} page analyses. Missing means not observed by the provider in this market.</p>
    ...
    <section class="space-y-4" aria-labelledby="competitor-opportunities-heading">
        ...
        <p class="text-slate-600 text-base sm:text-sm">Observed page features inform these briefs; hypotheses about ranking success are not proven causes. Use original writing and verified facts.</p>
        @forelse ($audit->opportunities as $opportunity)
            <article class="ui-panel space-y-4 p-4 sm:p-6">
                <h3 class="font-semibold text-slate-950">{{ $opportunity->title }}</h3>
                <p class="text-slate-600 text-base sm:text-sm">{{ $opportunity->brief['relevance_reason'] ?? '' }}</p>
                <p class="text-slate-600 text-base sm:text-sm">{{ $opportunity->brief['content_format'] ?? '' }} · {{ $opportunity->brief['search_intent'] ?? '' }}</p>
                <p class="break-words text-slate-600 text-base sm:text-sm">Target: {{ $opportunity->brief['existing_page_url'] ?: 'Choose after checking existing coverage' }}</p>
                ...
                @if ($opportunity->content_request_id)<p class="font-medium text-teal-700 text-base sm:text-sm">Queued in Content</p>
                @elseif ($canManageWebsite && ! $audit->competitor->excluded)<form method="POST" action="{{ route('admin.competitor-opportunities.queue', [$website, $opportunity]) }}">@csrf<button type="submit" class="ui-button ui-button-primary">Queue content brief</button></form>@endif
            </article>
        @empty
            <p class="rounded-xl border bg-white p-6 text-slate-500 text-base sm:text-sm">No content briefs yet. Briefs need relevant keywords and successfully analysed pages; limited evidence may produce no recommendations.</p>
        @endforelse
    </section>
    <section class="ui-panel p-4 sm:p-6" aria-labelledby="competitor-keywords-heading">
        <h2 id="competitor-keywords-heading" class="text-lg font-semibold">Keywords</h2>
        <nav class="my-4 flex flex-wrap gap-3 text-sm" aria-label="Keyword filters">@foreach (['all' => 'All keywords', 'missing' => 'Missing keywords', 'outranked' => 'They outrank us'] as $value => $label)<a href="{{ route('admin.competitor-audits.show', [$website, $audit, 'filter' => $value]) }}" @if ($filter === $value) aria-current="page" @endif class="ui-tab">{{ $label }}</a>@endforeach</nav>
    ...
    <section class="space-y-4" aria-labelledby="competitor-pages-heading">
        ...
        @empty<p class="text-slate-500 text-base sm:text-sm">No leading pages returned yet.</p>@endforelse
    </section>
</div>
@endsection
