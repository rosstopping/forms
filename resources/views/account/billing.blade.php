...
<div class="space-y-8">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><p class="font-medium text-teal-700 text-base sm:text-sm">Account</p><h1 class="text-2xl font-semibold text-slate-950">Billing and membership</h1><p class="mt-1 max-w-2xl text-slate-600 text-base sm:text-sm">Choose the level of website care that fits your business. Stripe securely handles payment details, invoices, package changes, and cancellation.</p></div>
        @if ($user->stripe_customer_id)<form method="POST" action="{{ route('admin.billing.portal') }}">@csrf<button type="submit" class="ui-button ui-button-secondary">Manage membership in Stripe</button></form>@endif
    </div>
    ...
    <section class="ui-panel ui-section" aria-labelledby="membership-status-title">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div><p class="font-semibold uppercase tracking-widest text-slate-500 text-base sm:text-sm">Current membership</p><h2 id="membership-status-title" class="mt-1 text-lg font-semibold text-slate-950">{{ data_get($plans, $user->effectiveMembershipTier().'.name', 'No active package') }}</h2><p class="mt-1 text-slate-600 text-base sm:text-sm">Status: <span class="font-medium capitalize">{{ $user->hasAdminManagedMembership() ? 'admin managed' : str_replace('_', ' ', $user->membership_status ?: 'not subscribed') }}</span>@if ($user->hasAdminManagedMembership() && $user->admin_membership_expires_at) · Admin-managed access ends {{ $user->admin_membership_expires_at->format('j F Y') }}@endif
                @if (! $user->hasAdminManagedMembership() && $user->membership_current_period_end) · Current period ends {{ $user->membership_current_period_end->format('j F Y') }}@endif</p>@if (! $user->hasAdminManagedMembership() && $user->membership_cancel_at)<p class="mt-1 font-medium text-amber-700 text-base sm:text-sm">Cancellation is scheduled for {{ $user->membership_cancel_at->format('j F Y') }}.</p>@endif</div>
            @if ($user->stripe_customer_id)<p class="text-slate-500 text-base sm:text-sm">Package changes and cancellation open securely on Stripe.</p>@endif
        </div>
    </section>
    ...
    <div class="grid gap-5 xl:grid-cols-3">
        @foreach ($plans as $tier => $plan)
            ...
            <article @class(['ui-panel ui-section flex flex-col', 'ring-2 ring-teal-700/20' => $tier === 'growth', 'border-slate-950/10' => $tier !== 'growth'])>
                ...
                <p class="mt-3 min-h-12 leading-6 text-slate-600 text-base sm:text-sm">{{ $plan['description'] }}</p>
                @if ($tier === 'growth' && $growthOffer = \App\Support\MembershipPlan::activeGrowthOffer())
                    <p class="mt-6 font-semibold uppercase tracking-widest text-teal-700 text-base sm:text-sm">20% off until 31 December 2026</p>
                ...
                @endif
                <p class="mt-3 font-semibold text-slate-700 text-base sm:text-sm">{{ $plan['summary'] }}</p>
                ...
                <form method="POST" action="{{ $user->hasActiveMembership() && $user->stripe_customer_id ? route('admin.billing.portal') : route('admin.billing.checkout') }}" class="mt-auto pt-7">
                    ...
                    <button type="submit" @disabled($current) @class(['ui-button w-full', 'ui-button-primary' => ! $current && $tier === 'growth', 'ui-button-secondary' => $current || $tier !== 'growth'])>{{ $current ? 'Current package' : ($user->stripe_customer_id ? 'Change with Stripe' : 'Choose '.$plan['name']) }}</button>
                </form>
            </article>
        @endforeach
    </div>
