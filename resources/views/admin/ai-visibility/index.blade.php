...
<div class="space-y-6">
    ...
    @else<div class="rounded-xl border border-dashed border-slate-950/10 p-8 text-center"><h2 class="font-semibold text-slate-950">Your first visibility baseline starts here</h2><p class="mt-2 text-slate-600 text-base sm:text-sm">@if (! $settings->enabled)Turn on AI tracking above to start your first checks.@elseif ($activePromptCount === 0)Add or reactivate a question to start your first checks.@else Tracking is on. Use Check now to queue eligible checks or see what is preventing them from starting. Results will appear here as checks complete.@endif</p></div>@endif
