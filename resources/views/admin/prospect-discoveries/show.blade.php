...
<div class="space-y-6">
    ...
    @elseif ($discovery->candidates->isEmpty())
        <div class="rounded-xl border border-slate-200 bg-white p-8 text-center"><p class="font-medium">No public business listings found</p><p class="mt-1 text-sm text-slate-500">Try a larger nearby city or a different business type.</p></div>
    @else
        <form method="POST" action="{{ route('admin.prospect-discoveries.import', $discovery) }}" class="space-y-4">
            @csrf
            <div class="flex flex-wrap items-center justify-between gap-3"><p class="text-sm text-slate-600">Businesses with websites are researched. Those without one become website opportunities. You still approve every email.</p><button class="rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800">Import selected</button></div>
