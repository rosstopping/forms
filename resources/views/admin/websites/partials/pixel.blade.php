    ...
    <section class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm" aria-labelledby="pixel-installation-title">
        ...
        @if ($canManageWebsite)
            <div class="flex flex-col gap-3 border-t border-slate-200 bg-slate-50 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div><p class="text-sm font-medium text-slate-900">Connection controls</p><p class="mt-1 text-xs text-slate-500">Pixel can be disabled from Website Settings. Doing so immediately removes every Pixel change from public payloads without deleting deployment history.</p></div>
                <div class="flex shrink-0 flex-wrap gap-2">
                    <a href="{{ route('admin.websites.show', ['website' => $website, 'tab' => 'settings']) }}" class="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">Open website settings</a>
