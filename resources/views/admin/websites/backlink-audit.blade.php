...
<div class="space-y-6">
    <header class="space-y-3">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div><h1 class="text-2xl font-semibold tracking-tight text-slate-950">Backlink audit</h1><p class="mt-1 break-all text-slate-600 text-base sm:text-sm">{{ $audit->domain }} · {{ ucfirst(str_replace('_', ' ', $audit->status)) }}</p></div>
            @if ($canManageWebsite)
                <form method="POST" action="{{ route('admin.backlink-audits.store', $website) }}">@csrf
                    ...
                    <button type="submit" class="ui-button ui-button-primary">{{ $audit->status === 'failed' ? 'Retry audit' : 'Refresh audit' }}</button>
                </form>
            @endif
        </div>
    ...
    <section class="ui-panel overflow-hidden" aria-labelledby="backlink-overview-heading">
        <div class="border-b p-4 sm:p-6"><h2 id="backlink-overview-heading" class="text-lg font-semibold text-slate-950">Overview</h2><p class="mt-1 text-slate-600 text-base sm:text-sm">Profile size and quality indicators from the provider sample.</p></div>
    ...
    <section class="ui-panel p-4 sm:p-6" aria-labelledby="backlink-opportunities-heading">
        <h2 id="backlink-opportunities-heading" class="text-lg font-semibold text-slate-950">Prioritised opportunities</h2>
        <p class="mt-1 text-slate-600 text-base sm:text-sm">Observations are separated from hypotheses. Content recommendations require original work and verified business facts.</p>
        <div class="mt-4 space-y-4">
            @forelse ($audit->opportunities as $opportunity)
                <article class="rounded-lg border p-4">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div><p class="font-semibold uppercase tracking-wide text-teal-700 text-base sm:text-sm">{{ str_replace('_', ' ', $opportunity->type) }} · Priority {{ $opportunity->priority_score }}</p><h3 class="mt-1 font-semibold text-slate-950">{{ $opportunity->title }}</h3><p class="mt-1 text-slate-600 text-base sm:text-sm">{{ $opportunity->evidence['user_need'] ?? $opportunity->evidence['relevance_reason'] ?? '' }}</p></div>
                        ...
                        @elseif ($opportunity->type === 'content' && $canManageWebsite)<form method="POST" action="{{ route('admin.backlink-opportunities.queue', [$website, $opportunity]) }}">@csrf<button type="submit" class="ui-button ui-button-primary">Queue content brief</button></form>@endif
                    </div>
            ...
            @empty
                <p class="rounded-lg bg-slate-50 p-4 text-slate-500 text-base sm:text-sm">No actionable opportunities have been produced from the available evidence yet.</p>
            @endforelse
        </div>
    </section>

    <section class="ui-panel p-4 sm:p-6" aria-labelledby="new-lost-heading">
        <h2 id="new-lost-heading" class="text-lg font-semibold">New and lost trend</h2><p class="mt-1 text-slate-600 text-base sm:text-sm">Monthly provider observations over the configured twelve-month window.</p>
    ...
    <section class="ui-panel p-4 sm:p-6" aria-labelledby="links-heading">
        <h2 id="links-heading" class="text-lg font-semibold">Backlinks</h2>
        <nav class="my-4 flex flex-wrap gap-2" aria-label="Backlink filters">@foreach (['all' => 'All', 'live' => 'Live', 'lost' => 'Lost', 'broken' => 'Broken'] as $value => $label)<a href="{{ route('admin.backlink-audits.show', [$website, $audit, 'links' => $value]) }}" @if ($linkFilter === $value) aria-current="page" @endif class="ui-tab">{{ $label }}</a>@endforeach</nav>
    ...
    <section class="ui-panel p-4 sm:p-6" aria-labelledby="pages-heading">
        <h2 id="pages-heading" class="text-lg font-semibold">Linked pages</h2><p class="mt-1 text-slate-600 text-base sm:text-sm">Own pages appear first, followed by sampled competitor pages. Only the strongest five competitor candidates are fetched for analysis.</p>
    ...
    <section class="ui-panel p-4 sm:p-6" aria-labelledby="gaps-heading">
        <h2 id="gaps-heading" class="text-lg font-semibold">Competitor link gaps</h2><p class="mt-1 text-slate-600 text-base sm:text-sm">Domains observed linking to selected competitors and not this website. Relevance still requires human review.</p>
