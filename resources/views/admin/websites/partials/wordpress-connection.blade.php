...
<section class="ui-panel p-4" aria-labelledby="wordpress-connection-title">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
            <div class="flex flex-wrap items-center gap-2">
                <p class="font-medium uppercase tracking-wide text-slate-500 text-base sm:text-sm">Static delivery</p>
            ...
            @if ($wordpressConnected)
                ...
                <p class="mt-1 text-slate-500 text-base sm:text-sm">Last checked in {{ $wordpressConnection->last_seen_at?->diffForHumans() ?: 'never' }}.@if ($wordpressConnection->last_deployed_at) Latest release went live {{ $wordpressConnection->last_deployed_at->diffForHumans() }}.@endif</p>
            @else
                <p class="mt-1 max-w-2xl text-slate-600 text-base sm:text-sm">Generate a one-time code here, then enter it under Settings → Sitewell Static Frontend in WordPress. The code expires after ten minutes.</p>
            @endif
        </div>
        ...
        @if ($canManageWebsite)
            <div class="flex shrink-0 flex-wrap gap-2">
                <form method="POST" action="{{ route('admin.websites.wordpress.pairing-code', $website) }}">
                    @csrf
                    <button type="submit" @disabled(! $website->repository) class="ui-button ui-button-primary disabled:cursor-not-allowed">
                ...
                @if ($wordpressConnection && ($wordpressConnected || $wordpressConnection->pairing_code_expires_at?->isFuture()))
                    <form method="POST" action="{{ route('admin.websites.wordpress.connection.destroy', $website) }}">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="ui-button ui-button-danger">Revoke</button>
                    </form>
                @endif
                @if ($wordpressConnected && $website->repository)
                    <form method="POST" action="{{ route('admin.websites.wordpress.releases.store', $website) }}">
                        @csrf
                        <button type="submit" @disabled($latestRelease && in_array($latestRelease->status, ['queued', 'building'], true)) class="ui-button ui-button-primary disabled:cursor-not-allowed">Deploy latest GitHub version</button>
                    </form>
                @endif
            </div>
        @endif
    ...
    @unless ($website->repository)
        <p class="mt-4 rounded-md bg-amber-50 px-3 py-2 text-amber-800 text-base sm:text-sm">Connect the website’s GitHub repository before connecting the WordPress plugin.</p>
    @endunless

    @if ($pairingCode)
        <div class="mt-4 rounded-lg border border-violet-200 bg-violet-50 p-4">
            <p class="font-medium text-violet-950 text-base sm:text-sm">Enter this code in the WordPress plugin</p>
            ...
            <p class="mt-2 text-violet-700 text-base sm:text-sm">This code is shown once and expires in ten minutes. Generating another code invalidates it.</p>
        </div>
    @endif

    @if ($website->wordpressStaticReleases->isNotEmpty())
        <div class="mt-4 border-t border-slate-950/10 pt-4">
            <p class="font-medium text-slate-900 text-base sm:text-sm">Recent static releases</p>
                ...
                @foreach ($website->wordpressStaticReleases as $release)
                    <div class="flex flex-wrap items-center justify-between gap-2 py-2 text-sm">
                        <div>
                            ...
                            @if ($release->error)
                                <p class="mt-1 max-w-2xl text-red-700 text-base sm:text-sm">{{ $release->error }}</p>
                            @endif
                        </div>
                        <span @class([
