...
- A queued audit checks the registered primary domain's homepage.
...
- Store report state, summary counts, category scores, check results, and comparison data in a `website_health_reports` table.
...
1. Crawl a bounded number of sitemap pages for broken links, duplicate metadata, canonical problems, and missing image alternatives.
2. Add PageSpeed Insights mobile/desktop data and Core Web Vitals when API configuration is available.
3. Integrate Google Search Console for impressions, clicks, query movement, and indexing information.
