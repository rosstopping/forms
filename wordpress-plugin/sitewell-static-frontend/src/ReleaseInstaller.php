...
final class ReleaseInstaller
{
    ...
    public function install( array $release, string $archivePath ): void
    {
        ...
        if (! wp_mkdir_p($temporaryPath) ) {
            throw new RuntimeException(esc_html__('WordPress could not create the static release directory.', 'sitewell-static-frontend'));
        }
        ...
        try {
            ...
            if (! rename($temporaryPath, $releasePath) ) {
                throw new RuntimeException(esc_html__('WordPress could not activate the static release directory.', 'sitewell-static-frontend'));
            }
    ...
    private function validateRelease( array $release, string $archivePath ): void
    {
        if (! preg_match('/^wsr_[a-z0-9]{28}$/', $release['release_id'])
            ...
            throw new RuntimeException(esc_html__('The downloaded static release failed verification.', 'sitewell-static-frontend'));
        }
    }
    ...
    private function extract( string $archivePath, string $destination ): void
    {
        if (! class_exists(ZipArchive::class) ) {
            throw new RuntimeException(esc_html__('The PHP ZIP extension is required to install static releases.', 'sitewell-static-frontend'));
        }

        $archive = new ZipArchive();

        if ($archive->open($archivePath) !== true ) {
            throw new RuntimeException(esc_html__('The static release is not a valid ZIP archive.', 'sitewell-static-frontend'));
        }
        ...
        try {
            if ($archive->numFiles > self::MAX_FILES ) {
                throw new RuntimeException(esc_html__('The static release contains too many files.', 'sitewell-static-frontend'));
            }
            ...
            for ( $index = 0; $index < $archive->numFiles; $index++ ) {
                ...
                if (! is_string($name) ) {
                    throw new RuntimeException(esc_html__('The static release contains an invalid file.', 'sitewell-static-frontend'));
                }
                ...
                if ($totalBytes > self::MAX_BYTES ) {
                    throw new RuntimeException(esc_html__('The extracted static release is too large.', 'sitewell-static-frontend'));
                }
            ...
            if (! $hasIndex || ! $archive->extractTo($destination) ) {
                throw new RuntimeException(esc_html__('The static release could not be extracted or has no index.html file.', 'sitewell-static-frontend'));
            }
        } finally {
            $archive->close();
        }
    ...
    private function assertSafePath( string $path ): void
    {
        ...
        if ($normalized === ''
            ...
            || in_array($extension, [ 'php', 'phtml', 'phar', 'cgi', 'pl', 'py', 'sh' ], true) 
        ) {
            throw new RuntimeException(esc_html__('The static release contains an unsafe file path.', 'sitewell-static-frontend'));
        }
    }
