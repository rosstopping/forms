<section class="space-y-6" aria-labelledby="content-activity-title">
            ...
            <div class="mt-5 border-t border-slate-950/10 pt-4">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">Requests that have been picked up, with their latest preparation or review status.</p>
                    </div>
                        ...
                        <article class="rounded-lg border border-emerald-200 bg-emerald-50/50 p-3">
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                ...
                                @if ($canManageWebsite && $contentRequest->generation?->pull_request_url)
                                    <a href="{{ $contentRequest->generation->pull_request_url }}" target="_blank" rel="noreferrer" class="shrink-0 rounded-md border border-emerald-300 bg-white px-2.5 py-1.5 text-sm font-medium text-emerald-800 hover:bg-emerald-100">View pull request</a>
                                @elseif ($canManageWebsite && $contentRequest->generation?->copilot_task_url)
                                    <a href="{{ $contentRequest->generation->copilot_task_url }}" target="_blank" rel="noreferrer" class="shrink-0 rounded-md border border-slate-300 bg-white px-2.5 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-50">View generation task</a>
                                @endif
                            </div>
                        </article>
                    @endforeach
    ...
    @if ($actionedContentRequests->isEmpty())
        <div class="rounded-lg bg-slate-50 p-5"><p class="font-medium text-slate-900">No requests have been picked up yet</p><p class="mt-1 text-base text-slate-600 sm:text-sm">Once Sitewell starts preparing a queued request, its progress appears here.</p></div>
    @endif
