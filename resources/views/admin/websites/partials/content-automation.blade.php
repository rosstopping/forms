<section class="rounded-2xl bg-white p-5 ring-1 ring-slate-200/70 sm:p-6" aria-labelledby="content-schedule-title">
    <div class="flex flex-wrap items-start justify-between gap-4">
        ...
        @if ($website->repository && Auth::user()?->isAdmin())
            <form method="POST" action="{{ route('admin.content-generations.store', $website) }}">
                ...
                <button type="submit" class="rounded-lg bg-teal-50 px-3 py-1.5 text-sm font-medium text-teal-800 hover:bg-teal-100 focus-visible:outline-teal-600">Generate now</button>
            </form>
        @endif
    </div>
    ...
    <form method="POST" action="{{ route('admin.content-plans.update', $website) }}" class="mt-6 space-y-6">
        ...
        <div class="rounded-xl bg-teal-50/70 p-4">
            <label for="content-schedule-enabled" class="flex items-center gap-3 bg-transparent text-base font-medium text-teal-950 hover:bg-transparent sm:text-sm">
        ...
        <div class="rounded-xl bg-slate-50/80 p-4 sm:p-5">
            ...
            <div class="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                <div>
                    ...
                    <select id="weekday" name="weekday" class="w-full border-0 shadow-none ring-slate-200" @if ($errors->has('weekday')) aria-invalid="true" @endif>
                    ...
                    @error('weekday')<p class="mt-2 text-sm text-rose-700">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="hour">Preparation time</label>
                    <select id="hour" name="hour" class="w-full border-0 shadow-none ring-slate-200" @if ($errors->has('hour')) aria-invalid="true" @endif>
                        @for ($hour = 0; $hour < 24; $hour++)
                            <option value="{{ $hour }}" @selected((int) old('hour', $contentPlan?->hour ?? 8) === $hour)>{{ str_pad($hour, 2, '0', STR_PAD_LEFT) }}:00</option>
                        @endfor
                    </select>
                    @error('hour')<p class="mt-2 text-sm text-rose-700">{{ $message }}</p>@enderror
                </div>
                <div class="sm:col-span-2 lg:col-span-1">
                    <label for="timezone">Timezone</label>
                    <input id="timezone" name="timezone" value="{{ old('timezone', $contentPlan?->timezone ?? 'Europe/London') }}" placeholder="Europe/London" required class="w-full border-0 shadow-none ring-slate-200" @if ($errors->has('timezone')) aria-invalid="true" @endif>
            ...
            @if ($contentWeeklyLimit === 3)
                <fieldset class="mt-5">
                    ...
                    <div class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4 xl:grid-cols-7">
                        @foreach (['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'] as $value => $day)
                            <label for="content-extra-day-{{ $value }}" class="flex items-center gap-2 rounded-lg bg-white px-3 py-2.5 text-base text-slate-600 ring-1 ring-slate-200/80 hover:bg-teal-50 has-checked:bg-teal-50 has-checked:text-teal-900 has-checked:ring-teal-600/30 sm:text-sm">
                    ...
                    @error('additional_weekdays')<p class="mt-2 text-sm text-rose-700">{{ $message }}</p>@enderror
                    @foreach ($errors->get('additional_weekdays.*') as $messages)
                        @foreach ($messages as $message)<p class="mt-2 text-sm text-rose-700">{{ $message }}</p>@endforeach
                    @endforeach
                </fieldset>
        ...
        <details class="rounded-xl bg-slate-50/80 p-4 sm:p-5" @if ($errors->has('audience') || $errors->has('guidance')) open @endif>
            ...
            <div class="mt-5 grid gap-5 md:grid-cols-2">
                <div>
                    ...
                    <textarea id="audience" name="audience" rows="5" maxlength="20000" placeholder="For example: Local homeowners comparing reliable, practical options for their next renovation." class="w-full border-0 shadow-none ring-slate-200" @if ($errors->has('audience')) aria-invalid="true" @endif>{{ old('audience', $contentPlan?->audience) }}</textarea>
                    @error('audience')<p class="mt-2 text-sm text-rose-700">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="guidance">How should your content sound?</label>
                    <textarea id="guidance" name="guidance" rows="5" maxlength="20000" placeholder="For example: Friendly and informative. Use British English, explain unfamiliar terms, and avoid unsupported claims." class="w-full border-0 shadow-none ring-slate-200" @if ($errors->has('guidance')) aria-invalid="true" @endif>{{ old('guidance', $contentPlan?->guidance) }}</textarea>
                    @error('guidance')<p class="mt-2 text-sm text-rose-700">{{ $message }}</p>@enderror
                </div>
            </div>
            <p class="mt-3 text-sm text-slate-400">Up to 20,000 characters in each field.</p>
        </details>
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <button type="submit" class="shrink-0 rounded-lg bg-teal-700 px-3 py-2 text-sm font-medium text-white hover:bg-teal-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-600">Save content plan</button>
        </div>
    </form>
</section>
